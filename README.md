...
### Аудио пайплайн и тонкости I2S на ESP32-S3

Путь сигнала от микрофона до нейронки:

```
INMP441 → I2S (32bit/16kHz) → DMA буфер (int32) → конвертация → WakeNet (int16)
```

**Формат INMP441:** микрофон выдаёт 24 значимых бита упакованных в 32-битное
слово (MSB-justified). Младший байт всегда 0x00/0x80 — это выравнивание ADC.
Правильный сдвиг для извлечения сигнала: `>> 12` (экспериментально подобран по
реальным данным).

**Скрытое чередование каналов:** ESP32-S3 в режиме `I2S_SLOT_MODE_MONO` всё
равно передаёт данные парами L+R в DMA буфер, где R = 0. Это не баг конфигурации
— это особенность аппаратного I2S контроллера. Обнаружено по сырым данным:
каждый нечётный int32 равен нулю. Фикс: читать `samples × 2` int32, брать только
чётные индексы.

**Масштаб сигнала:** WakeNet рассчитана на сигнал с RMS около -26dBFS.
`get_vol_gain()` возвращает 10.0 — микрофон на 20dB тише оптимума. Это типично
для INMP441 без AFE.
...
0xF1000    600KB   model (SPIFFS)   Веса одной WakeNet9.
...
model,      data,  spiffs,   0xF1000,   1138K
```
...
**Почему 1138 КБ?**: потому что я в примере использовал 4 модели wakenet и
инструмент movemodel рекомендовал мне разметить 1138 КБ для раздела model
...
- Реализован и отлажен полный аудио пайплайн: I2S → int32 → int16 конвертация →
  WakeNet inference
- Отлажена конвертация сэмплов: обнаружено чередование L/R каналов в MONO режиме
  ESP32-S3, правильный сдвиг >> 12
- Разметка model раздела: movemodel.py рекомендовал 1138KB для 4 моделей, раздел
  расширен с 600KB до 1138KB
- WakeNet детекция работает: все 4 модели (Alexa, Computer, Hi ESP, Jarvis)
  корректно распознают `wake words` в реальном времени:

| ![wake word detection demo](./docs/wakenet-detect-4.png) |
| -------------------------------------------------------- |

### В работе

- Замеры потребления через INA219: active inference vs light sleep vs deep sleep
- Реализация light sleep между фреймами
