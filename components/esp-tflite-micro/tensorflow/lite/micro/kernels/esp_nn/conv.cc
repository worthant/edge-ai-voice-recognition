...
static TfLiteStatus Prepare(TfLiteContext *context, TfLiteNode *node) {
    ...
    TF_LITE_ENSURE_STATUS(
        CalculateOpDataConv(context, node, params, input_width, input_height,
                            ...
                            output_height, input->type, &data->op_data));
    ...
    if (filter->type == kTfLiteInt4) {
        ...
        context->RequestScratchBufferInArena(
            context, filter_size, &data->op_data.filter_buffer_index);
    }

#if ESP_NN
    if (input->type == kTfLiteInt8) {
        data_dims_t input_dims = {.width = input_width,
                                  .height = input_height,
                                  .channels = input_channels,
                                  .extra = 1};
        data_dims_t output_dims = {.width = output_width,
                                   .height = output_height,
                                   .channels = output->dims->data[3],
                                   .extra = 1};
        data_dims_t filter_dims = {.width = filter_width,
        ...
        conv_params_t conv_params = {
            ...
            .padding = {data->op_data.padding.width,
...
inline void EvalQuantizedPerChannel(TfLiteContext *context, TfLiteNode *node,
    ...
    if (dilation_width_factor == 1 && dilation_height_factor == 1) {
        ...
        for (int i_batch = 0; i_batch < batch_size; i_batch++) {
            esp_nn_conv_s8(
                &input_dims, input_data + i_batch * input_size, &filter_dims,
        ...
        case kTfLiteInt4: {
            int8_t *unpacked_filter_data =
                static_cast<int8_t *>(context->GetScratchBuffer(
                    context, data.op_data.filter_buffer_index));
